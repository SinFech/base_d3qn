...
from model import ConvDQN, ConvDuelingDQN
...
class Agent:
    ...
    def test(self, env_test, model_name=None, path=None):
        ...
        for t in tqdm(range(len(env_test.data))):
            action = self.select_action(state)
        ...
        # Storing the action and the corresponding price
            current_price = state[-1].item()
    ...
    # Plotting
        ...
        plt.plot(prices, label='Price', color='gray', alpha=0.5)  # Plot prices
        scatter_colors = ['red' if a == 2 else 'green' if a == 1 else 'blue' for a in actions]
        plt.scatter(range(len(actions)), prices, c=scatter_colors, label='Actions', edgecolor='none', alpha=0.7)
        red_patch = mpatches.Patch(color='red', label='Sell')
        green_patch = mpatches.Patch(color='green', label='Buy')
        blue_patch = mpatches.Patch(color='blue', label='Hold')
        plt.legend(handles=[red_patch, green_patch, blue_patch])
        plt.xlabel('Time Step')
        plt.ylabel('Price')
        plt.title('Action Taken at Each Price Over Time')
        plt.show()
